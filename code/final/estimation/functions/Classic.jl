...
export lin_classic, sym_classic
...
function calibrate()
	...
	return @save σᶻ β σ η ϵ ρᶠ h ρᵇ s m γ gsY
...
function steady_state(cal::Dict{Symbol,Float64})
	...
	function SS!(R,x)
		...
		R[2] = γ/((1-η)*ϕ*q)  - int(zc)
		R[3] = b - (ρᵇ*wp + h)

	end

	R = ones(3)
	sol = nlsolve( SS!, [0.6599821461285086, 0.9709811707623278, 0.8337464323037389], show_trace=true, ftol=1e-12, iterations=20)
	zp = sol.zero[1]
	θ = sol.zero[2]
	b = sol.zero[3]
	...
	ss_y = @save c zp θ ϕ F b wp v Y zc π F_A
...
function set_lin_mod!(f_y_tp1, f_y_t, f_x_tp1, f_x_t, param, cal, ss_x, ss_y)
	...
	@index c zp θ ϕ F b wp v Y zc π F_A
	...
	f_y_t[10,zc] = zc_ss
	f_y_t[10,zp] = -zp_ss
	f_y_t[10,F] = -F_ss/ϕ_ss
	f_y_t[10,ϕ] = F_ss/ϕ_ss
	f_x_tp1[10,A] = F_ss/ϕ_ss
	...
	f_y_t[12,c] = β*(1-s)*(A_ss*ϕ_ss*int(zp_ss) - F_ss)
	...
	f_y_t[13,wp] = wp_ss
	f_x_tp1[13,A] = -η*((1-s)*ϕ_ss*I(zp_ss) + ϕ_ss*m*θ_ss^(-σ)*v_ss*I(zc_ss)/np_ss)
	f_y_t[13,ϕ] = f_x_tp1[13,A]
	f_x_t[13,np] = -η*((1-s)*ϕ_ss*I(zp_ss) + F_ss*(1-s)*(1-CDF(zp_ss)))
	f_y_t[13,zp] = η*(1-s)*ϕ_ss*zp_ss^2*PDF(zp_ss) + η*F_ss*(1-s)*zp_ss*PDF(zp_ss)
	f_y_t[13,θ] = η*ϕ_ss*m*θ_ss^(-σ)*v_ss*I(zc_ss)*σ/np_ss - η*γ*θ_ss
	f_y_t[13,v] = -η*ϕ_ss*m*θ_ss^(-σ)*v_ss*I(zc_ss)/np_ss
	f_y_t[13,zc] = η*ϕ_ss*m*θ_ss^(-σ)*v_ss*zc_ss^2*PDF(zc_ss)/np_ss
	f_y_t[13,F] = -η*F_ss*(1-s)*(1-CDF(zp_ss))
	f_y_t[13,c] = η*β*(1-s)*F_ss
	f_y_tp1[13,c] = -f_y_t[13,c]
	f_y_tp1[13,F] = η*β*(1-s)*F_ss
	f_y_t[13,b] = -(1-η)*b_ss
	f_x_tp1[13,np] = η*((1-s)*ϕ_ss*I(zp_ss) + (1-s)*(1-CDF(zp_ss))*F_ss + ϕ_ss*m*θ_ss^(-σ)*v_ss*I(zc_ss)/np_ss)

	#Definition of b

	f_y_t[14,b] = b_ss
	f_y_t[14,wp] = -ρᵇ*wp_ss

	#Definition of F

	f_y_t[15,F] = F_ss*(1-ρᶠ*η)
	f_y_t[15,b] = -ρᶠ*(1-η)*b_ss
	f_y_t[15,θ] = -ρᶠ*η*γ*θ_ss
	f_y_tp1[15,F] = ρᶠ*η*β*(1-s)*F_ss
	f_y_t[15,c] = f_y_tp1[15,F]
	f_y_tp1[15,c] = -f_y_tp1[15,F]
	f_y_t[15,ϕ] = -ρᶠ*η*ϕ_ss*I(zp_ss)/(1-CDF(zp_ss))
	f_x_tp1[15,A] = f_y_t[15,ϕ]
	f_y_t[15,zp] = -ρᶠ*η*ϕ_ss*int(zp_ss)*zp_ss*PDF(zp_ss)/(1-CDF(zp_ss))^2
...
function sym_mod(param, cal, ss_x, ss_y)
	...
	@vars c_t_hat zp_t_hat θ_t_hat ϕ_t_hat F_t_hat b_t_hat wp_t_hat v_t_hat Y_t_hat zc_t_hat π_t_hat F_A_t_hat
	@vars c_tp1_hat zp_tp1_hat θ_tp1_hat ϕ_tp1_hat F_tp1_hat b_tp1_hat wp_tp1_hat v_tp1_hat Y_tp1_hat zc_tp1_hat π_tp1_hat F_A_tp1_hat
	...
	Eq[13] = wp_t - η*( (A_t*(1-s)*ϕ_t*I(zp_t) + (1-ξ_t)*F_t)*np_tm1 + A_t*ϕ_t*q_t*v_t*I(zc_t) )/np_t - η*( -β_t_tp1*(1-s)*F_tp1 + γ*θ_t ) - (1-η)*b_t

	Eq[14] = b_t - ρᵇ*wp_t - h

	Eq[15] = F_t - ρᶠ*(η*(A_t*ϕ_t*I(zp_t)/(1-CDF(zp_t)) + F_t - β_t_tp1*(1-s)*F_tp1 + γ*θ_t) + (1-η)*b_t)

	Eq[16] = A_t_hat - F_A_t_hat
	...
	yp = [c_tp1_hat, zp_tp1_hat, θ_tp1_hat, ϕ_tp1_hat, F_tp1_hat, b_tp1_hat, wp_tp1_hat, v_tp1_hat, Y_tp1_hat, zc_tp1_hat, π_tp1_hat, F_A_tp1_hat]
	y = [c_t_hat, zp_t_hat, θ_t_hat, ϕ_t_hat, F_t_hat, b_t_hat, wp_t_hat, v_t_hat, Y_t_hat, zc_t_hat, π_t_hat, F_A_t_hat]
