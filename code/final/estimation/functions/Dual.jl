...
export lin_dual, sym_dual, @spec_funs, steady_state, calibrate, calibrate_wo_idx
...
"Calibrates the model without indexing of F and b over the wages."
function calibrate_wo_idx()
	...
	return @save σᶻ β σ η ϵ F h ρᵇw s δ ρ m γ gsY
...
function steady_state(x0, cal::Dict{Symbol,Float64})
	...
	ss_y = @save c zp zf θ ϕ F b wp wf v Y zs π F_A
...
function set_lin_mod!(f_y_tp1, f_y_t, f_x_tp1, f_x_t, param, cal, ss_x, ss_y)
	...
	@index c zp zf θ ϕ F b wp wf v Y zs π F_A
	@load_cat ss ss_x np nf R g A ϵ_A ϵ_μ ϵ_g ϵ_m
	@load_cat ss ss_y c zp zf θ ϕ F b wp wf v Y zs π F_A
	...
	f_y_t[11,ϕ] = F_ss/ϕ_ss
	f_x_tp1[11,A] = F_ss/ϕ_ss
	...
	f_y_t[15,wp] = wp_ss
	f_x_tp1[15,A] = -η*((1-s)*ϕ_ss*I(zp_ss) + ϕ_ss*m*θ_ss^(-σ)*v_ss*I(zs_ss)/np_ss)
	...
	f_x_tp1[16,A] = f_y_t[16,ϕ]
	f_x_tp1[16,nf] = -f_y_t[16,ϕ]
	...
	f_x_tp1[17,np] = np_ss*(b_ss - (ρᵇ*wp_ss+h))
	f_x_tp1[17,nf] = nf_ss*(b_ss - (ρᵇ*wf_ss+h))
...
function sym_mod(param, cal, ss_x, ss_y)
	...
	@vars c_t_hat zp_t_hat zf_t_hat θ_t_hat ϕ_t_hat F_t_hat b_t_hat wp_t_hat wf_t_hat v_t_hat Y_t_hat zs_t_hat π_t_hat F_A_t_hat
	@vars c_tp1_hat zp_tp1_hat zf_tp1_hat θ_tp1_hat ϕ_tp1_hat F_tp1_hat b_tp1_hat wp_tp1_hat wf_tp1_hat v_tp1_hat Y_tp1_hat zs_tp1_hat π_tp1_hat F_A_tp1_hat
	...
	Eq[13] = A_t*ϕ_t*zp_t - b_t + F_t - β_t_tp1*(1-s)*F_tp1 + β_t_tp1*F_A_tp1*ϕ_tp1*(1-s)*int(zp_tp1) - η*γ*θ_t/(1-η)

	Eq[14] = ρ*A_t*ϕ_t*zf_t - b_t + ρ*β_t_tp1*(1-δ)*F_A_tp1*ϕ_tp1*(Ez-zf_tp1) - η*γ*θ_t/(1-η)
	...
	yp = [c_tp1_hat, zp_tp1_hat, zf_tp1_hat, θ_tp1_hat, ϕ_tp1_hat, F_tp1_hat, b_tp1_hat, wp_tp1_hat, wf_tp1_hat, v_tp1_hat, Y_tp1_hat, zs_tp1_hat, π_tp1_hat, F_A_tp1_hat]
	y = [c_t_hat, zp_t_hat, zf_t_hat, θ_t_hat, ϕ_t_hat, F_t_hat, b_t_hat, wp_t_hat, wf_t_hat, v_t_hat, Y_t_hat, zs_t_hat, π_t_hat, F_A_t_hat]
...
function lin_dual()
	cal = calibrate()
	ss_x, ss_y = steady_state([0.64], cal)
...
# %%
"Defines the linear model with a Dual Labor Market"
function lin_dual(x0, cal)
	ss_x, ss_y = steady_state(x0, cal)
	param_names = ["ρ_A", "ρ_g", "ρ_R", "ρ_π", "ρ_y", "ψ", "σ_A", "σ_μ", "σ_g", "σ_m"]
	return LinMod{eltype(values(cal))}(cal, ss_x, ss_y, 4, 10, param_names, (f_y_tp1, f_y_t, f_x_tp1, f_x_t, param)->set_lin_mod!(f_y_tp1, f_y_t, f_x_tp1, f_x_t, param, cal, ss_x, ss_y), (G, R, Z, g_x, h_x)->state_space!(G, R, Z, g_x, h_x, ss_x))
end
# %%
