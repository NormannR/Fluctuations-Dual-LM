...
# data = DataFrame(load("input\\data.xlsx", "LONG2!B1:E70"))
# T = size(data,1)
# m = 4
# h = 8
# p = 4
# Filtered,_ = HamiltonFilter(data[:,[:n,:mufsmu]],h,p)
...
# h = 8
# p = 4
# Filtered,_ = HamiltonFilter(data[:,[:Y,:n]],h,p)
# Filtered = convert(Array{Float64,2},Filtered)
# Y = zeros(T-(p+h)+1,4)
# Y[:,[1,4]] = Filtered
# Y[:,2:3] = convert(Array{Float64,2},data[p+h:end,[:pi,:R]]) - mean(convert(Array{Float64,2},data[p+h:end,[:pi,:R]]))
...
# data = DataFrame(load("input\\data.xlsx", "STABLE!A1:E70"))
# T = size(data,1)
# m = 4
# h = 8
# p = 4
# Filtered,_ = HamiltonFilter(data[:,[:Y,:mufsmu]],h,p)
# Filtered = convert(Array{Float64,2},Filtered)
# Y = zeros(T-(p+h)+1,4)
# Y[:,[1,4]] = Filtered
# Y[:,2:3] = convert(Array{Float64,2},data[p+h:end,[:pi,:R]]) - mean(convert(Array{Float64,2},data[p+h:end,[:pi,:R]]))
# figure()
# plot(Y)
# legend(["Y/n","pi","R","mufsmu"])
#
# data = DataFrame(load("input\\data.xlsx", "SHORT!A1:E52"))
# T = size(data,1)
# m = 4
# h = 8
# p = 4
...
@everywhere f(x1,x2) = Likelihoods!(x1,x2,param,indposest,Y,funcmod,bound,ss,dGAM0,dGAM1,dGAM2,dGAM3,dA,dB,dO,dSIG0,dSIG,J1,J2)
...
# JLD.save("SHORT.jld","theta",drawsMat[Nphi,:,:],"weights",weightsMat[:,Nphi],"loglh",loglh,"logpost",logpost,"constMat",constMat)
JLD.save("LT_SHORT.jld","theta",drawsMat[Nphi,:,:],"weights",weightsMat[:,Nphi],"loglh",loglh,"logpost",logpost,"constMat",constMat)
# JLD.save("LONG.jld","theta",drawsMat[Nphi,:,:],"weights",weightsMat[:,Nphi],"loglh",loglh,"logpost",logpost,"constMat",constMat)
