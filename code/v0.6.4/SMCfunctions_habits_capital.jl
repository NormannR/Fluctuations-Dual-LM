function logprior(paramest)
	...
	prior[6] = logpdf(Beta(alpha,beta),paramest[6])
	prior[10] = logpdf(Beta(alpha,beta),paramest[10])
	prior[12] = logpdf(Beta(alpha,beta),paramest[12])
	prior[14] = logpdf(Beta(alpha,beta),paramest[14])
	prior[16] = logpdf(Beta(alpha,beta),paramest[16])
	prior[18] = logpdf(Beta(alpha,beta),paramest[18])

	prior[4] = logpdf(Truncated(Normal(1.5,0.75),1,Inf),paramest[4])
	prior[5] = logpdf(Truncated(Normal(0.5,0.75),0,Inf),paramest[5])
    ...
    prior[7] = logpdf(InverseGamma(alpha,beta),paramest[7])
    prior[8] = logpdf(InverseGamma(alpha,beta),paramest[8])
    prior[9] = logpdf(InverseGamma(alpha,beta),paramest[9])
	prior[15] = logpdf(InverseGamma(alpha,beta),paramest[15])
	prior[17] = logpdf(InverseGamma(alpha,beta),paramest[17])
	prior[19] = logpdf(InverseGamma(alpha,beta),paramest[19])

	prior[11] = logpdf(Truncated(Normal(1.5,0.4),0,Inf),paramest[11])
	prior[13] = logpdf(Truncated(Normal(4.,1.5),0,Inf),paramest[13])
...
function PriorDraws(priorMat::Array{Float64,2},f)
    ...
    @sync @parallel for n in 1:Npart
        ...
        while !valid
			...
			priorMat[n,[1,2,3,6,10,12,14,16,18]] = rand(BBeta,9)
            priorMat[n,4] = rand(Truncated(Normal(1.5,0.75),1,Inf))
			priorMat[n,5] = rand(Truncated(Normal(0.5,0.75),0,Inf))
			priorMat[n,[7,8,9,15,17,19]] = rand(IG,6)
			priorMat[n,11] = rand(Truncated(Normal(1.5,0.4),0,Inf))
			priorMat[n,13] = rand(Truncated(Normal(4,1.5),0,Inf))
            try
                f(priorMat[n,:],0)
                valid = true
            end
...
function Likelihoods!(x,phi,param,posest,Y,funcmod,bound,ss)
...
function steady_state_eval(param)
    ...
    return [ 1+r, c, np, nf, zp, zf, zs, theta, phi, pi, Y, v, k, kp, nu, qk, rk, mc, X, I, A, mu, c, zp, zf, theta, phi, pi, qk, I, 1., 1., g]
...
function linearized_model_ss(param,ss)
    ...
    GAM0[2,lambda] = -(b - ss[phi]*ss[zp] - ss[phi]*F)
    GAM0[2,E_lambda] = (b - ss[phi]*ss[zp] - ss[phi]*F)
    ...
    GAM0[3,lambda] = -(b - rho*ss[phi]*ss[zf])
    GAM0[3,E_lambda] = (b - rho*ss[phi]*ss[zf])
    ...
    GAM0[9,E_pi] = - beta
    GAM0[9,mu] = - 1
    GAM0[9,mc] = - (1-beta*psi)*(1-psi)/psi
	...
	GAM0[22,lambda] = (1-h)*(1-beta*h)
	GAM0[22,E_c] = -sig_c*beta*h
	GAM1[22,c] = sig_c*h
	GAM0[22,c] = sig_c*(1+beta*h^2)
	GAM0[22,s_b] = -(1-h)*(1-beta*h*rho_b)
	...
	GAM0[27,qk] = 1
	GAM0[27,E_qk] = -beta*(1-delta)
	GAM0[27,E_rk] = -(1-beta*(1-delta))
	GAM0[27,r] = 1
	GAM0[27,E_pi] = -1

	GAM0[28,i] = 1
	GAM1[28,i] = 1/(1+beta)
	GAM0[28,qk] = -1/eta_k/(1+beta)
	GAM0[28,s_i] = -1
	GAM0[28,E_i] = -beta/(1+beta)
    ...
    stdev = [sig_A,sig_mu,sig_m,sig_g,sig_b,sig_i]
    SDX = diagm(stdev)
