function logprior(paramest)
	...
	prior[13] = logpdf(InverseGamma(alpha,beta),paramest[13])
...
function PriorDraws(priorMat::Array{Float64,2},f)
    ...
    @sync @parallel for n in 1:Npart
        ...
        while !valid
			...
			priorMat[n,[1,2,3,12]] = rand(bbeta,4)
			priorMat[n,6] = rand(bbeta_psi)
            priorMat[n,4] = rand(Truncated(Normal(1.5,0.75),1,Inf))
			priorMat[n,5] = rand(Truncated(Normal(0.5,0.75),0,Inf))
			priorMat[n,7:9] = rand(IG,3)
			priorMat[n,13] = rand(IG)
			priorMat[n,10] = rand(bbeta_h)
...
function steady_state_eval(param)
    ...
    return [ 1+i, c, np, nf, zp, zf, zs, theta, phi, pi, Y, v, A, mu, c, zp, zf, theta, phi, pi, g]
...
function linearized_model_ss(param,ss)
    ...
    stdev = [sig_A,sig_mu,sig_m,sig_g]
    SDX = diagm(stdev)
