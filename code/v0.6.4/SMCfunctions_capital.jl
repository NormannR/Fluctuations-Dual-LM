function logprior(paramest)
	...
	prior[11] = logpdf(Beta(alpha,beta),paramest[11])
	prior[13] = logpdf(Beta(alpha,beta),paramest[13])
	prior[15] = logpdf(Beta(alpha,beta),paramest[15])
	prior[17] = logpdf(Beta(alpha,beta),paramest[17])
	...
	prior[14] = logpdf(InverseGamma(alpha,beta),paramest[14])
	prior[16] = logpdf(InverseGamma(alpha,beta),paramest[16])
	prior[18] = logpdf(InverseGamma(alpha,beta),paramest[18])

	prior[10] = logpdf(Truncated(Normal(1.5,0.4),0,Inf),paramest[10])
	prior[12] = logpdf(Truncated(Normal(4.,1.5),0,Inf),paramest[12])
...
function PriorDraws(priorMat::Array{Float64,2},f)
    ...
    @sync @parallel for n in 1:Npart
        ...
        while !valid
			...
			priorMat[n,[1,2,3,6,11,13,15,17]] = rand(BBeta,8)
            priorMat[n,4] = rand(Truncated(Normal(1.5,0.75),1,Inf))
			priorMat[n,5] = rand(Truncated(Normal(0.5,0.75),0,Inf))
			priorMat[n,[7,8,9,14,16,18]] = rand(IG,6)
			priorMat[n,10] = rand(Truncated(Normal(1.5,0.4),0,Inf))
			priorMat[n,12] = rand(Truncated(Normal(4,1.5),0,Inf))
            try
                f(priorMat[n,:],0)
                valid = true
            end
