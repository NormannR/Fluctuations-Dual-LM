...
@everywhere begin
    ...
    include("input\\InitialGuess_capital.jl")
    include("SMCfunctions_capital.jl")
...
data = DataFrame(load("input\\data.xlsx", "SHORT_LT_ALL!I1:N45"))
Y = convert(Array{Float64,2},data)
plot(Y)
legend(["Y","pi","R","n","C","I"])

# data = DataFrame(load("input\\data.xlsx", "ALL_SHOCKS!A1:G95"))
# T = size(data,1)
# m = 4
# h = 8
# p = 4
# Filtered,_ = HamiltonFilter(data[:,[:Y,:n,:C,:I]],h,p)
...
# JLD.save("LONG_habits_capital.jld","theta",drawsMat[Nphi,:,:],"weights",weightsMat[:,Nphi],"loglh",loglh,"logpost",logpost,"constMat",constMat)
JLD.save("LT_SHORT_capital.jld","theta",drawsMat[Nphi,:,:],"weights",weightsMat[:,Nphi],"loglh",loglh,"logpost",logpost,"constMat",constMat)
