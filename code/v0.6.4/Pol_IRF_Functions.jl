...
function POL_linearized_model_ss(param,ss)
        ...
        GAM0[2,A] = ss[phi]*(ss[zp] + F*(1-beta*(1-s)*rho_F*rho_A) + beta*(1-s)*rho_A*int(ss[zp]))
        GAM0[2,phi] = ss[phi]*(ss[zp]+F)
        GAM0[2,eta_F] = ss[phi]*F*(1-beta*(1-s)*rho_F*rho_A)
        ...
        stdev = [sig_A,sig_mu,sig_g,sig_m,sig_F]
        SDX = diagm(stdev)
