...
function steady_state_eval(param)
    ...
    return [ 1+i, c, np, nf, zp, zf, zs, theta, phi, pi, Y, v, A, mu, c, zp, zf, theta, phi, pi]
...
function linearized_model_ss(param,ss)
	...
	GAM0[22,lambda] = 1
	GAM0[22,E_c] = -sig_c*beta*h/(1-beta*h)/(1-h)
	GAM1[22,c] = sig_c*h/(1-beta*h)/(1-h)
	GAM0[22,c] = sig_c*(1+beta*h^2)/(1-beta*h)/(1-h)
    ...
    ZZ[1,np] = +ss[np]/(ss[np]+ss[nf])
    ZZ[1,nf] = +ss[nf]/(ss[np]+ss[nf])
