...
@everywhere function solve_sims!(RR,GG,eu,Γ₀,Γ₁,Ψ,Π; tol = 1e-8)
...
@everywhere function logprior(paramest)
...
@everywhere function Likelihoods!(x,l,Y,bound,param,Γ₀,Γ₁,Ψ,Π,SDX,GG,RR,ZZ,eu)
...
@everywhere function kffast(y,Z,s,P,T,R)
...
function PriorDraws!(drawsMat,f)

    Npart = size(drawsMat,2)
    ...
    @sync @distributed for n in 1:Npart
        valid = false
        while !valid

			drawsMat[1,n,[1,2,3,4]] .= rand(B,4)
			drawsMat[1,n,7] = rand(B_ψ)
            drawsMat[1,n,5] = rand(Truncated(Normal(1.5,0.75),1,Inf))
			drawsMat[1,n,6] = rand(Truncated(Normal(0.5,0.75),0,Inf))
			drawsMat[1,n,8:11] .= rand(IG,4)

			valid = !isinf(f(drawsMat[1,n,:],0)[3])
...
@everywhere function MutationRWMH(p0, l0, post0, tune, i, f)
...
function SMCIter!(c, acpt, weightsMat, constMat, id, drawsMat, logpost, loglh, nresamp, tune, cMat, ESSMat, acptMat, rsmpMat, acpt_part, Nphi, Npart, Nparam, phi,f, trgt, mu, sig)
    ...
    for i in 2:Nphi
        ...
        z         = (para - repeat(sum(para.*wght,dims=1), Npart, 1))
        R       = (z.*wght)'*z
        ...
        @sync @distributed for j in 1:Npart
...
function SMC(;Nblock = 1,Npart = 1000,Nphi = 100,lambda = 2)
    ...
    @everywhere phi = ((0:(Nphi-1))/(Nphi-1)).^lambda

    #Storing the results

    drawsMat = SharedArray{Float64}(Nphi, Npart, Nparam)
    weightsMat   = zeros(Npart, Nphi)
    constMat    = zeros(Nphi)
    id = SharedArray{Int64}(Npart)
    loglh   = SharedArray{Float64}(Npart)
    logpost = SharedArray{Float64}(Npart)
    nresamp = 0
    ...
    acptMat = zeros(Nphi)
    rsmpMat = zeros(Nphi)
    acpt_part = SharedArray{Float64}(Npart)
    @everywhere tune = Array{Any,1}(undef,4)

    @everywhere f(x1,x2) = Likelihoods!(x1,x2,Y,bound,param,Γ₀,Γ₁,Ψ,Π,SDX,GG,RR,ZZ,eu)

    SMCInit!(weightsMat, constMat, drawsMat, logpost, loglh, Npart, phi, f)
    @time SMCIter!(c, acpt, weightsMat, constMat, id, drawsMat, logpost, loglh, nresamp, tune, cMat, ESSMat, acptMat, rsmpMat, acpt_part, Nphi, Npart, Nparam, phi,f, trgt, mu, sig)

	return (drawsMat[Nphi,:,:], weightsMat[:,Nphi], mu, sig)
